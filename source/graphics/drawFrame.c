...
static void update(struct Model model, uint32_t currentFrame) {
    for (uint32_t i = 0; i < model.instanceCount; i += 1) {
        ...
        glm_rotate(model.instanceBuffer[i].modelMatrix, model.instance[i].baseRotation[0], (vec3) { 1, 0, 0 });
        glm_rotate(model.instanceBuffer[i].modelMatrix, model.instance[i].baseRotation[1], (vec3) { 0, 1, 0 });
        glm_rotate(model.instanceBuffer[i].modelMatrix, model.instance[i].baseRotation[2], (vec3) { 0, 0, 1 });
